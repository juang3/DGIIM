...
//	 this.ground.material.transparent = true;
  ...
  getPointOnGround (event) {
    ...
    // Distinguiendo regiones (por estar la pantalla dividida)
    if(0.75< mouse.x && 0.75< mouse.y){
      this.raycaster.setFromCamera (mouse, scene.getCamera_primera_persona());}
  ...
  moveBox (event, action) {
	 switch (action) {
		...
		case TheScene.SELECT_BOX :
		  var mouse = this.getMouse (event);
		  this.raycaster.setFromCamera (mouse, scene.getCamera_general());
